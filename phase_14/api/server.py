...
class RequestHandler(
    ...
    def log_message(self, format, *args):
        # We use debug level so normal requests don't spam production logs
        # but are visible in development
...
def start_server():
    ...
    log.info(f"Server starting in {config.env.upper()} mode")
    log.info(f"Listening on http://{config.HOST}:{config.PORT}")
